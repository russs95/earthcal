...
/*
|--------------------------------------------------------------------------
| get_user_items.php — Earthcal v1.0
|--------------------------------------------------------------------------
| PURPOSE:
|   Returns *all* calendars visible to a given user — including:
|     - Personal Earthcal calendars they own
|     - Subscribed Earthcal calendars (shared/public)
|     - Synced iCal calendars (webcal)
|     - Public calendars (if include_public=true)
|
|   For each calendar, it retrieves all items (events, todos, journals)
|   from items_v1_tb that are active and not deleted.
|
| AUTH:
|   Requires a valid buwana_id passed in POST.
|   No JWT/session verification here — assumed front-end already knows
|   the user ID (trusted app context).
|--------------------------------------------------------------------------
*/
...
/*------------------------------------------------------------
| Allowed origins for security (CORS whitelist)
-------------------------------------------------------------*/
$allowed_origins = [
    'https://ecobricks.org',
    'https://earthcal.app',
...
/*------------------------------------------------------------
| Parse JSON body (fallback to form POST)
-------------------------------------------------------------*/
...
/*------------------------------------------------------------
| Required: buwana_id identifies the current user
-------------------------------------------------------------*/
...
/*------------------------------------------------------------
| Optional filters: include_public, only_active, year
-------------------------------------------------------------*/
$includePublicRaw = $payload['include_public'] ?? true;

/**
 * Helper closure for normalizing boolean inputs
 */
$boolFilter = static function ($value, bool $default = true): bool {
    if ($value === null) return $default;
    if (is_bool($value)) return $value;
    if (in_array($value, [1, '1', 'true', 'TRUE', 'on'], true)) return true;
    if (in_array($value, [0, '0', 'false', 'FALSE', 'off'], true)) return false;
    return $default;
};
...
/*------------------------------------------------------------
| Simple error logger helper for debugging
-------------------------------------------------------------*/
...
/*------------------------------------------------------------
| Connect to the Earthcal database
-------------------------------------------------------------*/
try {
...
/*------------------------------------------------------------
| Prepare array to collect all calendars
-------------------------------------------------------------*/
$calendars = [];

/**
 * Helper closure that ensures a calendar entry exists in the array,
 * merges subscription data (color, emoji, is_active flags),
...
$ensureCalendar = static function (array $row, string $sourceType, array $overrides = []) use (&$calendars, $boolFilter, $buwanaId): void {
    ...
    // Choose color/emoji from subscription override or calendar defaults
    ...
    // Create or merge calendar entry
    if (!isset($calendars[$calendarId])) {
        ...
        if (!empty($colorOverride)) $cal['color'] = $colorOverride;
        if (!empty($emojiOverride)) $cal['emoji'] = $emojiOverride;
        if ($subId) $cal['subscription_id'] = (int)$subId;
        if (isset($activeRaw)) $cal['is_active'] = $boolFilter($activeRaw, $cal['is_active']);
        if (isset($displayRaw)) $cal['display_enabled'] = $boolFilter($displayRaw, $cal['display_enabled']);
        if ($sourceType !== 'public' && $cal['source_type'] === 'public') $cal['source_type'] = $sourceType;
    }
};

/*------------------------------------------------------------
| 1. PERSONAL CALENDARS (owned by the user)
| Includes those marked as personal or created by the user.
-------------------------------------------------------------*/
$log('Executing personal calendar lookup', ['params' => ['uid' => $buwanaId]]);
...
");
$personalStmt->execute(['uid_join' => $buwanaId, 'uid_filter' => $buwanaId]);
foreach ($personalStmt as $row) {
    $ensureCalendar($row, 'personal');
}

/*------------------------------------------------------------
| 2. EARTHCAL SUBSCRIPTIONS (shared calendars)
| These are calendars shared by other users via Earthcal.
-------------------------------------------------------------*/
...
$earthcalStmt = $pdo->prepare("
    SELECT s.subscription_id, s.is_active AS sub_is_active, s.display_enabled AS sub_display_enabled,
      ...
      JOIN calendars_v1_tb AS c ON c.calendar_id = s.earthcal_calendar_id
     WHERE s.user_id = :uid
...
/*------------------------------------------------------------
| 3. PUBLIC CALENDARS (optional inclusion)
| These are globally visible calendars.
-------------------------------------------------------------*/
if ($includePublic) {
    $log('Executing public calendar lookup');
    $publicStmt = $pdo->prepare("SELECT c.* FROM calendars_v1_tb AS c WHERE c.visibility = 'public'");
    $publicStmt->execute();
...
/*------------------------------------------------------------
| Filter calendars for which to fetch items
-------------------------------------------------------------*/
$calendarIdsForItems = [];
...
/*------------------------------------------------------------
| 4. FETCH ITEMS for all collected calendars
-------------------------------------------------------------*/
$itemCount = 0;
if (!empty($calendarIdsForItems)) {
    ...
    // Main query: retrieve all active items for these calendars
    ...
    $log('Executing item lookup', ['calendar_ids' => $calendarIdsForItems]);
    $itemStmt = $pdo->prepare($query);
    $itemStmt->execute($params);

    // Loop through items and assign to corresponding calendar
    while ($row = $itemStmt->fetch(PDO::FETCH_ASSOC)) {
        if (!empty($row['deleted_at']) && $row['deleted_at'] !== '0000-00-00 00:00:00') continue;
        $calendarId = (int)$row['calendar_id'];
        if (!isset($calendars[$calendarId])) continue;
...
echo json_encode([
    ...
    http_response_code(500);
    $log('Unhandled error', ['exception' => $e->getMessage()]);
    echo json_encode([
        'ok' => false,
        'error' => 'server_error',
        'detail' => $e->getMessage(),
