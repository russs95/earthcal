...
function updateDayIds(year) {
  ...
  const dayPaths = document.querySelectorAll(
    'path[id$="-day"]:not([id*="lunar"]), path[id$="-day-marker"]:not([id*="lunar"])',
  );
...
function updateDayTitles(year) {
  const dayPaths =
    document.querySelectorAll('path[id$="-day"]'); /*Changed from $*/
  dayPaths.forEach((path) => {
...
function displayMoonPhaseInDiv(date) {
  ...
  const moonAngle = (moonPosition.parallacticAngle * (180 / Math.PI)).toFixed(
    2,
  );
  ...
  const per_MoonDist =
    ((moonDistance - minMoonDist) / (maxMoonDist - minMoonDist)) * 100;
...
// Handles hover and click on a day path
function handleDayPathMouseOver(event) {
  ...
  // Always update the moon phase for the hovered date
  updateMoonPhase(date);

  // On click, set the target date and refresh related UI
  if (event.type === "click") {
    targetDate = date;
    startDate = targetDate;
    displayDayInfo(targetDate, userLanguage, userTimeZone);
...
// Handles the event when a user touches off a Day path
