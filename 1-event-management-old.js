...
function findMatchingDateCycles(dateCycles) {
  ...
  // Filter and map to include month name in each matching dateCycle
  return dateCycles
      .filter(dc => dashedDate.includes(dc.Date))
      .map(dc => ({...dc, monthName: monthsNames[month - 1]}));
}
...
function writeMatchingDateCycles(divElement, dateCycle) {
  divElement.innerHTML += `
      <div class="date-info ${dateCycle.ID}">
          <div class="current-date-info-title ${dateCycle.monthName}">${dateCycle.Event_name}</div>          
          <div class="current-datecycle-data">
              <div class="current-date-calendar-color" style="color:${dateCycle.calendar_color};">⬤</div>
...
<<<<<<< HEAD
          <div class="forward-button-datecycle" title="➡️ Push unto tomorrow" onclick="closeDatecycleInfo(this)">⇢</div>
=======
          <div class="forward-button-datecycle" title="➡️ Push to tomorrow" onclick="push2tomorrow('${dateCycle.ID}')">➜</div>
>>>>>>> 9eb28c7 (hh)
          <div class="close-button-datecycle" title="✅ Done! Hide." onclick="closeDatecycleInfo(this)">✔</div>
